...
class Server_GPRS_worker implements Runnable
{
    ...
    Server_GPRS_worker(Logger logger, Socket socket) throws Exception
    {
        this.logger = logger;
    ...
    private void run0() throws Exception
    {
        ...
        while (m_isRuning)
        {
            ...
            if (inputStream.available() == 0)
            {
                ...
                logger.debug(String.format(">>>GPRS服务(%s), 收到:%s", m_clientIdentity, info));
                ...
                logger.debug(String.format(">>>GPRS服务(%s), 生成的响应内容: %s", m_clientIdentity, responseStr));
    ...
    public void run()
    {
        try {
            ...
            logger.debug(String.format(">>>GPRS服务(%s), 读取超时", m_clientIdentity));
        } catch (Exception e) {
            logger.debug(String.format(">>>GPRS服务(%s), 读取异常: %s", m_clientIdentity, e.getMessage()));
        }
        ...
        try {
            ...
            logger.debug(String.format(">>>GPRS服务(%s), 关闭 socket时出现错误:%s", m_clientIdentity, e.getMessage()));
        }
    }
}
...
{
    private static final Logger logger = Logger.getLogger(Server_GPRS.class);
    ...
    public Server_GPRS(int port) throws Exception
    {
    ...
    {
        if(m_isRuning) {
            logger.error(">>>GPRS服务启动失败，该服务正在执行!");
        } else {
            m_isRuning = true;
            m_thread = new Thread(this::run);
            m_thread.setDaemon(true);
            m_thread.setName("Server_GPRS thread");
            m_thread.start();
            logger.debug(">>>GPRS服务启动成功...");
            return true;
        }
        return false;
    }
    ...
    private void run()
    {
        while (m_isRuning)
        {
            try {
                ...
                Server_GPRS_worker worker = new Server_GPRS_worker(logger, socket);
                ...
                e.printStackTrace();
                logger.debug(">>>GPRS服务, 未知错误: " + e.getMessage());
                break;
            }
        }
        m_isRuning = false;
